                         ...
                         void *arg)
{
        ...
        if (hc->addr_conf->quic)
    {
        ...
        if (h3scf->enable && h3scf->enable_hq)
        {
            ...
            srvlen = sizeof(NGX_HTTP_V3_ALPN_PROTO NGX_HTTP_V3_HQ_ALPN_PROTO) - 1;
        }
...
ngx_http_ssl_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_bitmask_value(conf->protocols, prev->protocols,
                                 (NGX_CONF_BITMASK_SET | NGX_SSL_TLSv1 | NGX_SSL_TLSv1_1 | NGX_SSL_TLSv1_2 | NGX_SSL_TLSv1_3));
    ...
    if (ngx_ssl_ciphers(cf, &conf->ssl, &conf->ciphers,
                        conf->prefer_server_ciphers) != NGX_OK)
    {
        return NGX_CONF_ERROR;
    }
    ...
    else if (conf->certificates)
    {
        ...
        if (ngx_ssl_certificates(cf, &conf->ssl, conf->certificates,
                                 conf->certificate_keys, conf->passwords) != NGX_OK)
        {
            return NGX_CONF_ERROR;
        }
    }
    ...
    if (conf->stapling)
    {
        ...
        if (ngx_ssl_stapling(cf, &conf->ssl, &conf->stapling_file,
                             &conf->stapling_responder, conf->stapling_verify) != NGX_OK)
        {
            return NGX_CONF_ERROR;
        }
    }
...
ngx_http_ssl_ocsp_cache(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (value[1].len <= sizeof("shared:") - 1 || ngx_strncmp(value[1].data, "shared:", sizeof("shared:") - 1) != 0)
    {
        goto invalid;
    }
